...
public class InQuireAdpter extends RecyclerView.Adapter<InQuireAdpter.InQuireViewHolder>{

    private List<InQuireBean.DataDTO> datas ;


    private Context context;

    public InQuireAdpter(List<InQuireBean.DataDTO> datas, Context context) {
        this.datas = datas;
    ...
    public List<InQuireBean.DataDTO> getDatas() {
        return datas;
    }

    public void setDatas(List<InQuireBean.DataDTO> datas) {
        this.datas = datas;
    }
    ...
    public void onBindViewHolder(@NonNull InQuireViewHolder holder, int position) {
        InQuireBean.DataDTO daily =datas.get(position);
        ...
        if (!(year + "").equals(daily.getCreatTime().substring(0, 4))) {
            holder.time.setText(daily.getCreatTime().substring(0, 10));
        }else if(!month_copy.equals(daily.getCreatTime().substring(5, 7))){
            holder.time.setText(daily.getCreatTime().substring(0, 10));
        }else if(!(day_copy+"").equals(daily.getCreatTime().substring(8,10))){
            holder.time.setText(daily.getCreatTime().substring(0, 10));
        }else{
            holder.time.setText("今天"+daily.getCreatTime().substring(11, 16));
        }
...
//                context.startActivity(intent);
                ...
                Constant.orivoaid = daily.getVoaid();
